...
export class MinimapCache {
  ...
  invalidate() { this.invalid = true; this.indexed = false; }
  beginPacket(field, count, snapshot) {
    ...
    if (count && !this.invalid && !this.indexed) {
  ...
  draw(field, paintBackground, paintBases, paintCell) {
    ...
    if (this.invalid || this.width !== w || this.signature !== signature || this.territories !== field.territories) {
      ...
      this.canvas ??= document.createElement('canvas'); this.canvas.width = this.canvas.height = w;
      this.context = this.canvas.getContext('2d'); this.indexed = false;
      this.background ??= document.createElement('canvas'); this.background.width = this.background.height = w;
