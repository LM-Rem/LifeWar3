...
if (cardGridEl) {
  ...
  function useCard(card) {
    ...
    if (isTargetedCard(cardObj)) {
      battlefield.cardTarget = { cardId, instanceId:cardObj.instanceId, name: cardObj.name, ...cardObj.effect };
      card.classList.remove('dragging');
  ...
  playCardAnimated = (card, target = {}) => {
    ...
    decomposeCard(card, () => {
      ...
      send({ type: 'play_card', cardId, instanceId:operation.instanceId, ...(target.x === undefined ? {} : {x:target.x,y:target.y}) }).then(sent => {
...
function onMessage(msg) {
  switch(msg.type){
    case 'performance':
      if (browserMetrics) {
        browserMetrics.record('computedGeneration', 0, msg.generation, '', String(msg.epoch));
        browserMetrics.record('sentGeneration', 0, msg.generation, '', String(msg.epoch));
      }
      break;
    case 'hello':break;
...
function renderCards(force = false) {
  ...
  for (const card of hand) {
    ...
    if (card.instanceId !== undefined) el.dataset.instanceId = card.instanceId;
...
function updateCardStatus() {
  ...
  const inheritance = (state.cards.effects || []).filter(e => e.stat === 'birthPriority');
  $('#inheritance-status').classList.toggle('hidden', !inheritance.length);
  $('#inheritance-status').textContent = inheritance.map(e=>`${state.players.find(p=>p.id===e.playerId)?.name} · 同化 ${seconds(e.endsAt)}s`).join(' / ') + (inheritance.length>1 ? ' · 冲突：普通归属' : '');
  const pressure=state.nodePressure;
  $('#node-pressure-status').textContent = pressure ? (now < pressure.startsAt ? `节点压制 ${formatClock(pressure.startsAt-now)} 后开启` : `节点压制中 · 敌方每节点 ${pressure.perNode} HP/s`) : '';
    ...
    if(!multiTouch&&drag&&!drag.moved){
      if(drag.button===0){
        if(battlefield.cardTarget){
          ...
          const card = [...cardGridEl.querySelectorAll('.card')].find(c => c.dataset.cardId === cardTarget.cardId && c.dataset.instanceId === cardTarget.instanceId);
