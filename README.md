...
发牌时间统一配置在 `public/cards.json` 的 `drawSeconds`，单位为开局后的秒数，默认 `[180, 300, 420]`。例如改为 `[60, 180, 300, 420, 600]`，即可在第 1、3、5、7、10 分钟发牌；支持任意轮数，时间须严格递增，`[]` 表示关闭。第 1、2 轮使用 early、mid 卡池，第 3 轮起均使用 late 卡池。修改后重启服务器并刷新页面，界面提示自动同步。

每轮提供法则、增益、道具各一张，30 秒内选择。手牌无持有上限，同名卡可重复持有；选择新卡追加到手牌，超时自动追加本轮增益卡。

点击顶部战斗栏展开仓库。PC 直接拖动卡牌；触屏先单击选中，再拖动选中卡牌。法则和增益拖出仓库后播放消散动画；道具拖出后点击战场选择目标，再在目标处播放消散动画。所有卡牌均在动画结束后请求服务器生效。选点时可用右键、Esc 或「取消选点」退出。
