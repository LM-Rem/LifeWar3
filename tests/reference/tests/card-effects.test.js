...
test('every law obeys its B/S truth table for all nonzero neighbor counts',()=>{
  for(const card of CARDS.filter(c=>c.effect.kind==='rule')) for(let n=0;n<=8;n++){
    const {g,clock,play}=setup();
...
test('energy refill uses the current cap; overload expires; same stat refreshes without stacking',()=>{
  const {g,clock,play}=setup();play('energy_overload');assert.equal(g.players[0].energy,120+Math.round(RULES.maxEnergy*.35));
  play('energy_overload');assert.equal(g.energyCap(1),RULES.maxEnergy+Math.round(RULES.maxEnergy*.25));
