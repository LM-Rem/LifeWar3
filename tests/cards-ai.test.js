...
test('卡池按时间：第 1/2/3 次发卡分别从 early/mid/late 池抽取', () => {
  ...
  for (let round = 0; round < 3; round++) {
    ...
    for (const e of g.cardDraft.players) {
      // 手牌上限 1：上一轮选中的卡先使用掉（道具卡在原点使用也能成功），才能选新卡
      const old = g.cards.hand[e.playerId - 1];
...
test('AI 自动参与三选一：bot 获得手牌且 draft 不被卡住', () => {
  ...
  assert.ok(g.cards.hand[1], 'bot 应获得手牌');
  ...
  // 第二次发卡：bot 手牌未用时自动替换旧卡，draft 不卡住
  advance(3);
  assert.ok(g.cardDraft, '第二次发卡应正常触发');
  assert.ok(g.cards.hand[1], 'bot 手牌应被替换为新卡');
});
...
test('AI 使用增益卡：立即生效并消耗手牌', () => {
  ...
  assert.equal(g.cards.hand[1], null, '使用后手牌应消耗');
});
...
test('并发：两名玩家同时选卡并各自使用手牌互不冲突', () => {
  ...
  const c1 = g.cards.hand[0], c2 = g.cards.hand[1];
  ...
  assert.equal(g.cards.hand[1], null);
});