...
try {
 ...
 mkdirSync(values.output,{recursive:true});writeFileSync(`${values.output}/raw.json`,JSON.stringify(results));assert.deepEqual(errors,[]);for(let i=0;i<2;i++)assert.deepEqual(results.current[i].hashes.map(({name,main})=>({name,main})),results.reference[i].hashes.map(({name,main})=>({name,main})),`DPR ${i+1}`);
 mkdirSync(values.output,{recursive:true});const sourceHashes=Object.fromEntries(['renderer.js','world-texture.js','minimap-cache.js','territory.js'].map(file=>[file,createHash('sha256').update(readFileSync(new URL('../../public/'+file,import.meta.url))).digest('hex')]));const report={sourceHashes,status:'PASS',browser:browser.version(),comparisons:results.current.reduce((n,r)=>n+r.hashes.length,0),scope:'main canvas headless lossless PNG equality; approximate minimap checked separately by overview-performance; timings exploratory single session, not frame-presentation certification',results};writeFileSync(`${values.output}/report.json`,JSON.stringify(report,null,2));console.log(JSON.stringify({status:report.status,comparisons:report.comparisons,timings:results.current.map(r=>({dpr:r.dpr,timings:r.timings}))}));
