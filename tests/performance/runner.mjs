...
const { values } = parseArgs({ options: { profile: { type: 'string', default: 'quick' }, scenario: { type: 'string', default: 'P01' },
  backend: { type: 'string', default: 'current' }, output: { type: 'string' }, generations: { type: 'string' }, rounds: { type: 'string' }, warmup: { type: 'string' },
  trace: { type: 'boolean', default: false }, help: { type: 'boolean', default: false } } });
if (values.help) {
  console.log('runner.mjs --profile smoke|quick|full --scenario P01|P03|P04 --backend current|legacy [--trace] [--generations N] [--rounds N] [--warmup N] [--output DIR]'); process.exit(0);
}
const profiles = { smoke: [10, 20, 1], quick: [20, 300, 3], full: [100, 1000, 5] };
if (!profiles[values.profile] || !['current', 'legacy'].includes(values.backend)) throw new Error('Invalid profile/backend; use --help');
...
const { Game, RULES } = await import(values.backend === 'legacy' ? '../reference/src/engine.js' : '../../src/engine.js');
...
for (const rel of ['src/engine.js', 'src/dormancy.js', 'src/bots.js', 'public/cards.js', 'public/cards.json', 'public/territory.js']) {
  const base = values.backend === 'legacy' ? '../reference/' : '../../';
  report.sourceHashes[rel] = createHash('sha256').update(readFileSync(new URL(base + rel, import.meta.url))).digest('hex');
}
